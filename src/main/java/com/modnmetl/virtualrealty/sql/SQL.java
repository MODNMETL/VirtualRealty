...
public class SQL {
        ...
        try {
            switch (VirtualRealty.getPluginConfiguration().dataModel) {
                case SQLITE: {
                    ...
                    File dataDir = new File(VirtualRealty.getInstance().getDataFolder().getAbsolutePath() + File.separator + "data");
                    if (!dataDir.exists()) {
                        dataDir.mkdirs();
                    }
                    connection = DriverManager.getConnection("jdbc:sqlite:" + VirtualRealty.getInstance().getDataFolder().getAbsolutePath() + File.separator + "data" + File.separator + "data.db");
                    break;
                }
                case MYSQL: {
    ...
    public static void closeConnection() {
        try {
            if (statement != null && (VirtualRealty.getPluginConfiguration().dataModel != PluginConfiguration.DataModel.SQLITE && !statement.isClosed())) {
                statement.close();
            }
            if (connection != null)
                connection.close();
