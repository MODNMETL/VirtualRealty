...
public class SQL {
    ...
    public static void connect() {
        try {
            if (VirtualRealty.getInstance().getConfig().getString("data-storage").equalsIgnoreCase("h2")) {
                ...
                //System.out.println("jdbc:h2:" + VirtualRealty.getInstance().getDataFolder().getAbsolutePath() + "\\data\\data");
            } else {
    ...
    public static void createTables() {
        try {
            SQL.getStatement().execute("CREATE TABLE IF NOT EXISTS `vr_plots` (`ID` INT(12) NOT NULL, `ownedBy` VARCHAR(36) NOT NULL, `assignedBy` VARCHAR(36) NOT NULL, `ownedUntilDate` DATETIME NOT NULL, `floorMaterial` VARCHAR(32) NOT NULL, `floorMaterialData` INT(32) NOT NULL, `plotSize` VARCHAR(32) NOT NULL, `length` INT(24) NOT NULL, `width` INT(24) NOT NULL, `height` INT(24) NOT NULL, `createdLocation` TEXT(500) NOT NULL, PRIMARY KEY(`ID`))");
        }
        catch (SQLException ex) {
            ex.printStackTrace();
        }
